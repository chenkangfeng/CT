...
public:
    ...
    void set_timeout(const timeval& time, const std::functrion<void(void)>& callback);
    ctr_bool asyn_parse(ctr_strptr domain, const std::functrion<void(address&)>& callback);
...
private:
    ...
    timeval time_;
    std::functrion<void(void)> timeout_callback_;
    
    std::mutex mutex_;
    std::thread thread_;
    std::functrion<void(address&)> parse_callback_;
};
